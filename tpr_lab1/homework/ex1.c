...
int main(int argc, char **argv)
{
    ...
    if (world_rank == 0)
    {
        ...
        for (i = 0; i < sendTimes; i++)
        {
            ...
            MPI_Recv(dataArray, arraySize, MPI_INT, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            double end = MPI_Wtime();
            double timeElapsed = end - start;
            printf("%f\n", arraySize * sizeof(int) / timeElapsed / mBit / 2);
        }
    }
    else if (world_rank == 1)
    {
        ...
        for (i = 0; i < sendTimes; i++)
        {
            ...
            MPI_Send(dataArray, arraySize, MPI_INT, 0, 0, MPI_COMM_WORLD);
        }
    }
